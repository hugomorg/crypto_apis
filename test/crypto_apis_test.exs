  ...
  describe "get!/2" do
    test "collects params from top-level" do
      with_mock HTTPoison,
        ...
        assert %HTTPoison.Response{} = response = CryptoApis.get!(@url, params: [key: :value])
        assert response.request.options == [params: [key: :value]]
      end
    end
    ...
    test "get! returns successful non-json responses" do
      with_mock HTTPoison,
        ...
        assert %HTTPoison.Response{} = response = CryptoApis.get!(@url)
        assert response.body == @encoded
        assert response.status_code == 200
      end
    end
    ...
    test "get! does not raise for error codes" do
      with_mock HTTPoison,
        get!: fn _url, _headers, _options ->
          error_response(status_code: 500)
        end do
        assert %HTTPoison.Response{status_code: 500} = CryptoApis.get!(@url)
      end
    end
